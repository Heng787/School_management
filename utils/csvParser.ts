...
import { Student, StudentStatus, Class, Staff, StaffRole } from '../types';

interface CSVParseResult {
    validStudents: Omit<Student, 'id'>[];
    errors: { row: number; message: string }[];
}

interface StaffCSVParseResult {
    validStaff: Omit<Staff, 'id'>[];
    errors: { row: number; message: string }[];
}

interface ClassCSVParseResult {
    validClasses: Omit<Class, 'id'>[];
    errors: { row: number; message: string }[];
}
 ...
 * Returns an empty string if dateStr is empty (optional fields).
 */
const normalizeDate = (dateStr: string, fieldName: string): string => {
    if (!dateStr) return '';
...
/**
 * Maps various common header names to the internal canonical key.
 */
/**
 * Maps various common header names for students to the internal canonical key.
 */
const getCanonicalHeader = (header: string): string | null => {
 ...
 */
const getStaffCanonicalHeader = (header: string): string | null => {
    const h = header.toLowerCase().replace(/[^a-z0-9]/g, '');
 ...
 */
const getClassCanonicalHeader = (header: string): string | null => {
    const h = header.toLowerCase().replace(/[^a-z0-9]/g, '');
...
export const parseStudentCSV = (csvContent: string): CSVParseResult => {
    ...
    if (missingColumns.length > 0) {
        const niceNames: Record<string, string> = { name: 'Full Name', sex: 'Sex' };
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        try {
            ...
            validStudents.push({
                ...
                enrollmentDate,
                status,
                // Initialize tuition property with zero values
 ...
 */
export const parseStaffCSV = (csvContent: string): StaffCSVParseResult => {
    const validStaff: Omit<Staff, 'id'>[] = [];
    const errors: { row: number; message: string }[] = [];
    ...
    if (missingColumns.length > 0) {
        const niceNames: Record<string, string> = { name: 'Name', role: 'Role', contact: 'Contact' };
    ...
    const shorthandMap: Record<string, StaffRole> = {
        'T': StaffRole.Teacher,
 ...
 */
export const parseClassCSV = (csvContent: string, staffList: Staff[]): ClassCSVParseResult => {
    const validClasses: Omit<Class, 'id'>[] = [];
    const errors: { row: number; message: string }[] = [];
    ...
    if (missingColumns.length > 0) {
        const niceNames: Record<string, string> = { name: 'Class Name', level: 'Level', schedule: 'Schedule' };
        const missingNice = missingColumns.map(c => niceNames[c] || c);
        errors.push({ row: 1, message: `Missing required columns: ${missingNice.join(', ')}` });
        return { validClasses, errors };
    }
