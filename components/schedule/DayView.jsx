...
const EVENT_CONFIG = {
  [EventType.Holiday]: {
    ...
    badge:
      "bg-rose-100 dark:bg-rose-900/50 text-rose-600 dark:text-rose-300 ring-1 ring-rose-200 dark:ring-rose-800/50",
    dot: "bg-rose-500",
    icon: "🎌",
  },
  [EventType.Meeting]: {
    ...
    badge:
      "bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300 ring-1 ring-blue-200 dark:ring-blue-800/50",
    dot: "bg-blue-500",
    icon: "📋",
  },
  [EventType.Exam]: {
    ...
    badge:
      "bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300 ring-1 ring-amber-200 dark:ring-amber-800/50",
    dot: "bg-amber-400",
    icon: "📝",
  },
  [EventType.General]: {
    ...
    badge:
      "bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 ring-1 ring-slate-200 dark:ring-slate-600",
    dot: "bg-slate-400",
    icon: "📌",
  },
};
    ...
    <div className="rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700/60 shadow-xl shadow-slate-200/60 dark:shadow-none bg-white dark:bg-slate-900 transition-colors duration-300">
      ...
      <div
        className={`px-6 py-5 border-b border-slate-200 dark:border-slate-700/60 flex items-center gap-5
          ${
            isToday
              ? "bg-linear-to-r from-primary-50 to-indigo-50 dark:from-primary-950/30 dark:to-indigo-950/30"
      ...
      >
        ...
        <div
          className={`w-16 h-16 rounded-2xl flex flex-col items-center justify-center shrink-0 shadow-md
            ${
              isToday
                ? "bg-linear-to-br from-primary-500 to-indigo-600 text-white shadow-primary-200 dark:shadow-primary-900/50"
      ...
      <div className="p-6">
        {dayEvents.length === 0 ? (
          ...
          <div className="space-y-3">
            {dayEvents.map((event) => {
              const cfg =
                EVENT_CONFIG[event.type] || EVENT_CONFIG[EventType.General];
              return (
                <button
                  key={event.id}
                ...
                >
                  ...
                  <div className="flex-1 p-4 flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`font-bold text-sm leading-tight ${cfg.title}`}
                      >
                        {event.title}
                      </p>
                      {event.description && (
                        <p className="text-xs opacity-60 mt-1 line-clamp-2 text-slate-600 dark:text-slate-300">
                          {event.description}
                        </p>
                      )}
                      <span
                        className={`inline-block mt-2 text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${cfg.badge}`}
                      >
                        {event.type}
                      </span>
                    </div>
                    <svg
                      className="w-4 h-4 opacity-30 shrink-0 mt-1"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
