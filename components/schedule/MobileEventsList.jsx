    ...
    <div className="sm:hidden mt-5 space-y-4">
      ...
      <div className="flex justify-between items-center px-1">
        ...
        <button
          ...
          className="text-xs font-bold text-white bg-linear-to-r from-primary-500 to-indigo-500 px-3 py-1.5 rounded-lg shadow-sm shadow-primary-200 dark:shadow-none active:scale-95 transition-all"
        >
          + Add
        </button>
      </div>
              ...
              >
                ...
                <div className="flex-1 p-3.5 flex items-center gap-3">
                  ...
                  <div className="grow min-w-0">
                    <p
                      className={`font-bold text-sm line-clamp-1 ${cfg.title}`}
                    >
                      {event.title}
                    </p>
                    <p className="text-xs line-clamp-1 mt-0.5 opacity-60 text-slate-600 dark:text-slate-300">
                      {event.description}
                    </p>
                    <span
                      className={`inline-block mt-1.5 text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-full ${cfg.badge}`}
                    >
                      {event.type}
                    </span>
                  </div>
                  <svg
                    className="w-4 h-4 opacity-30 ml-2 shrink-0"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
