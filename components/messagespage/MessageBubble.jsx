...
const FileAttachment = ({ file, isMine }) => (
  ...
  >
    <div
      className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${isMine ? "bg-white text-primary-600" : "bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400"}`}
    >
      <svg
        className="w-5 h-5"
        fill="none"
    ...
    <div className="min-w-0">
      <p
        className={`text-sm font-black truncate ${isMine ? "text-white" : "text-slate-800 dark:text-slate-100"}`}
      >
        {file.fileName || "Document"}
      </p>
      <p
        className={`text-[9px] font-black uppercase tracking-widest ${isMine ? "text-white/60" : "text-slate-500"}`}
      >
        Download
      </p>
    </div>
...
const MessageBubble = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex flex-col ${isMine ? "items-end" : "items-start"} max-w-[85%] sm:max-w-[70%] ${!isMine && "ml-3"}`}
      >
        {!isMine && (
          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 ml-1">
            {msg.senderName}
          </span>
        )}
          ...
          <div className="flex flex-col gap-2 w-full min-w-[280px]">
            ...
            <div className="flex gap-2 justify-end">
              <button
                ...
                className="px-4 py-2 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-slate-600"
              >
                Cancel
              </button>
              <button
                onClick={handleSaveEdit}
                className="px-5 py-2 bg-primary-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all"
              >
                Save Changes
              </button>
            </div>
          ...
          <div className={bubbleClasses}>
            ...
            {msg.metadata?.imageUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block mb-3 overflow-hidden rounded-xl border border-black/5 dark:border-white/5 shadow-sm hover:shadow-md transition-shadow"
              >
                <img
                  src={msg.metadata.imageUrl}
                  alt="attachment"
                  className="max-w-full sm:max-w-[300px] object-cover"
                />
              </a>
            )}
                  ...
                  {msg.metadata.attachments.map((at, i) =>
                    at.type === "image" ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="block overflow-hidden rounded-xl border border-black/5 dark:border-white/5 shadow-sm hover:shadow-md transition-all"
                      >
                        <img
                          src={at.url}
                          alt={at.name}
                          className="max-w-full sm:max-w-[320px] object-cover"
                        />
                      </a>
                    ) : (
                      <FileAttachment
                        key={i}
                        file={{ fileUrl: at.url, fileName: at.name }}
                        isMine={isMine}
                      />
                    ),
                  )}
            ...
            {msg.content && (
              <p className="text-[13px] sm:text-sm font-medium leading-relaxed whitespace-pre-wrap">
                {msg.content}
              </p>
            )}

            {msg.metadata?.edited && (
              <span className="text-[9px] font-bold opacity-40 mt-1 block uppercase tracking-tighter">
                Edited
              </span>
            )}
          </div>
          ...
          msg.metadata?.status === "pending" && (
            <div className="flex gap-2 mt-2 ml-1">
              <button
                ...
                className="px-4 py-1.5 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95 transition-all"
              >
                Approve
              </button>
              <button
                onClick={() => onStatusChange(msg.id, "rejected")}
                className="px-4 py-1.5 bg-rose-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-rose-500/20 active:scale-95 transition-all"
              >
                Reject
              </button>
            </div>
        ...
        {/* METADATA & ACTIONS */}
        <div
          className={`flex items-center gap-3 mt-1.5 px-1 transition-all duration-300 ${isMine ? "flex-row-reverse" : "flex-row"}`}
        >
          <span className="text-[9px] font-black text-slate-500 uppercase tracking-tighter opacity-80">
            {formatTime(msg.createdAt)}
          </span>
          {isMine && (
            <span
              className={`text-[10px] font-black ${msg.isRead ? "text-primary-500" : "text-slate-300"}`}
            >
              {msg.isRead ? "✓✓" : "✓"}
            </span>
          )}
