...
const StatCard = ({
  ...
  const colors = {
    ...
    primary:
      "text-primary-600 bg-primary-50 dark:bg-primary-900/20 border-primary-100 dark:border-primary-800/30",
    amber:
    ...
    <Card className="hover:border-primary-500/50" title={title} action={action}>
      <div className="flex justify-between items-start">
        <div className="min-w-0">
          ...
          {trend && (
            <div
              className={`mt-3 flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-bold w-fit border ${
                trendType === "up"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <StatCard
          title={
            attView === "daily" ? "Daily Attendance" : "Monthly Attendance"
          }
          ...
          action={
            <button
              onClick={() =>
                setAttView(attView === "daily" ? "monthly" : "daily")
              }
          ...
          icon={
            ...
            >
              <path
                ...
                strokeLinejoin="round"
                strokeWidth="2"
                d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04M12 2.944V21m0-18.056L3.382 5.984m8.618-3.04l8.618 3.04M12 21.056L3.382 5.984M12 21.056l8.618-15.072"
              />
            </svg>
          }
        />
              ...
              {recentActivity.map((log) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-slate-700 dark:text-slate-300 font-medium wrap-break-words">
                      {log.action}
                    </p>
...
export const TeacherDashboard = ({ navigate }) => {
  ...
  const myStudentsCount = enrollments.filter((e) =>
    myClassIds.has(e.classId),
  ).length;
  ...
  const myStudentsEnrollments = enrollments.filter((e) =>
    myClassIds.has(e.classId),
  );
  ...
  const { avgGrade, gradeBadge } = useMemo(() => {
    const teacherGrades = (grades || []).filter((g) =>
      myClassIds.has(g.classId),
    );
    if (teacherGrades.length === 0)
      return { avgGrade: "None", gradeBadge: null };

    const sum = teacherGrades.reduce(
    ...
    const getGradeInfo = (score) => {
      if (score >= 9.5)
        return { label: "A+", badge: "Academic Peak", color: "primary" };
      if (score >= 9.0)
        return { label: "A", badge: "Academic Peak", color: "primary" };
      if (score >= 8.5)
        return { label: "A-", badge: "Academic Peak", color: "primary" };
      if (score >= 8.0)
        return { label: "B+", badge: "Solid Performance", color: "emerald" };
      if (score >= 7.5)
        return { label: "B", badge: "Solid Performance", color: "emerald" };
      if (score >= 7.0)
        return { label: "B-", badge: "Satisfactory", color: "amber" };
      if (score >= 6.5)
        return { label: "C+", badge: "Satisfactory", color: "amber" };
      if (score >= 6.0)
        return { label: "C", badge: "Satisfactory", color: "amber" };
      if (score >= 5.0)
        return { label: "D", badge: "Needs Focus", color: "rose" };
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-rose-100 font-bold mt-1 uppercase tracking-widest text-[10px]">
                  You have {overdueReports.length} reports from previous months
                  pending submission
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <Card className="p-5! bg-linear-to-br from-primary-600 to-primary-700 text-white border-0 shadow-lg shadow-primary-100 dark:shadow-none hover:scale-[1.02] transition-all duration-500 group">
          <p className="text-primary-100/80 text-[10px] font-black uppercase tracking-[0.15em] mb-1.5 transition-colors">
            Total Students
          </p>
        ...
        <Card className="p-5! transition-all duration-500 hover:-translate-y-1 group">
          <p className="text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-[0.15em] mb-1.5 transition-colors">
        ...
        <Card className="p-5! transition-all duration-500 hover:-translate-y-1">
          ...
          {gradeBadge && (
            <div
              className={`mt-4 text-[10px] font-bold text-${gradeBadge.color}-600 bg-${gradeBadge.color}-50 px-2 py-0.5 rounded-full w-fit border border-${gradeBadge.color}-100 uppercase tracking-widest`}
            >
              {gradeBadge.badge}
            </div>
          )}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {myClasses.map((c) => {
              ...
              const draftsForClass = draftGrades
                ? draftGrades.filter((g) => g.classId === c.id)
                : [];
              return (
                <Card
                  key={c.id}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-2">
                      <div className="flex justify-between text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">
                        <span>Grade Mix</span>
                        <span>
                          {Math.round(Math.random() * 20 + 70)}% Passing
                        </span>
                      </div>
                      <div className="w-full h-2.5 flex rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 p-0.5 border border-slate-200/50 dark:border-slate-700 shadow-inner">
                        <div
                          className="bg-emerald-500 rounded-l-full shadow-sm"
                          style={{ width: "40%" }}
                        ></div>
                        <div
                        ...
                        ></div>
                        <div
                          className="bg-rose-500 rounded-r-full shadow-sm"
                          style={{ width: "10%" }}
                        ></div>
                      </div>
                    </div>
                      ...
                      <div className="flex space-x-2">
                        ...
                        <button
                          ...
                          disabled={
                            ...
                            isSending[`${c.id}_attendance`] ||
                            (draftAttendance || []).filter(
                              (a) => a.classId === c.id,
                            ).length === 0
                          }
                        ...
                        >
                          {isSending[`${c.id}_attendance`]
                            ...
                            : sentStatus[`${c.id}_attendance`]
                              ? "Sent!"
                              : `Submit Monthly (${(draftAttendance || []).filter((a) => a.classId === c.id).length})`}
                        </button>
                        ...
                        <button
                          ...
                          disabled={
                            ...
                            isSending[`${c.id}_grades`] ||
                            draftsForClass.length === 0
                          }
                          className={`flex-1 py-2 rounded-lg text-[10px] font-bold flex items-center justify-center transition-all ${
                            sentStatus[`${c.id}_grades`]
                              ? "bg-emerald-100 text-emerald-700 border border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400"
                        ...
                        >
                          {isSending[`${c.id}_grades`]
                            ...
                            : sentStatus[`${c.id}_grades`]
                              ? "Sent!"
                              : `Send Grades (${draftsForClass.length})`}
                        </button>
                      </div>
                    </div>
                  </div>
                    ...
                    tasks
                      ...
                      .map((task) => (
                        ...
                        >
                          ...
                          <span
                            className={`shrink-0 px-2 py-0.5 text-[9px] font-black uppercase rounded-full tracking-tighter ${
                              task.done
              ...
              {myClasses.length > 0 ? (
                myClasses.map((c, i) => {
                  ...
                  if (i === 0) {
                    ...
                    status = "Now";
                    statusColor =
                      "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 relative after:absolute after:top-0 after:right-0 after:-mr-1 after:-mt-1 after:w-2 after:h-2 after:bg-blue-500 after:rounded-full after:animate-ping";
                  }
                    ...
                    >
                      ...
                      <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/50 shadow-sm">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span
                            className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded ${statusColor}`}
                          >
                            {status}
                          </span>
                        </div>
            ...
            <div className="space-y-4">
              {[
                ...
                },
                {
                  ...
                  time: "Mon, 9:00 AM",
                  desc: "Requested student transfer",
                  icon: "💬",
                  color:
                ...
                <div key={i} className="flex space-x-4">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${act.color}`}
                  >
                    {act.icon}
                  </div>
                  <div>
                    ...
                    <p className="text-[10px] uppercase font-bold tracking-wider text-slate-500 mt-1.5">
                      {act.time}
                    </p>
                  </div>
                </div>
...
export const OfficeWorkerDashboard = () => {
  ...
  useEffect(() => {
    import("../../../services/messageService").then((m) => {
      m.fetchMessages(currentUser?.id, currentUser?.role === "Admin").then(
        (msgs) => {
          const recent = msgs
            .filter(
              (msg) =>
  ...
  const todayLogs = activityLogs.slice().reverse().slice(0, 15);
          ...
          <Card title="Enrollment Queue">
            <div className="space-y-3 max-h-[360px] overflow-y-auto custom-scrollbar pr-1">
              {pendingStudents.map((s) => (
                <div
                  key={s.id}
                  className="flex items-center justify-between p-3 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 hover:bg-white dark:hover:bg-slate-900 transition-colors"
                >
                  <div className="flex items-center gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card title="Recent Messages">
              <div className="space-y-4 max-h-56 overflow-y-auto custom-scrollbar pr-1">
                {messages.map((msg) => (
                  <div
                    key={msg.id}
                    className="relative pl-4 before:absolute before:left-0 before:top-2 before:bottom-2 before:w-1 before:rounded-full before:bg-slate-200 dark:before:bg-slate-700"
                  >
                    ...
                    {msg.type === "leave_request" && (
                      <span className="inline-block mt-1 text-[10px] font-bold uppercase text-amber-500 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded">
                        Leave Request
                      </span>
                    )}
                  </div>
            ...
            <Card title="My To-Dos">
              <div className="space-y-2 max-h-44 overflow-y-auto custom-scrollbar pr-1">
                {tasks
                  ...
                  .map((t) => (
                    <label
                      key={t.id}
                      className="flex items-start space-x-3 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-900 cursor-pointer transition-colors"
                    >
                      <input
                        type="checkbox"
                        checked={t.done}
                      ...
                      />
                      <span
                        className={`text-sm font-medium ${t.done ? "text-slate-500 line-through" : "text-slate-700 dark:text-slate-300"}`}
                      >
                        {t.text}
                      </span>
                    </label>
              ...
              <form onSubmit={handleAddTask} className="mt-3 flex gap-2">
                ...
                <button
                  type="submit"
                  className="px-3 py-2 bg-primary-600 text-white text-xs font-bold rounded-lg hover:bg-primary-500 transition-colors"
                >
                  Add
                </button>
              </form>
            ...
            <div className="space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-1">
              {onDutyStaff.map((s) => (
                <div key={s.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${s.status === "On Duty" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-slate-100 text-slate-500 dark:bg-slate-800"}`}
                    >
                      {s.name.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <p
                        className={`text-sm font-bold leading-tight ${s.status === "On Duty" ? "text-slate-800 dark:text-slate-200" : "text-slate-500 dark:text-slate-500"}`}
                      >
                        {s.name}
                      </p>
