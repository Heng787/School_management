...
// ─── STYLES ───────────────────────────────────────────────────────────────────
const inputCls =
  "mt-1 w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all";
const selectCls =
  "mt-1 block w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all sm:text-sm appearance-none cursor-pointer bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2020%2020%22%3E%3Cpath%20stroke%3D%22%236b7280%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20stroke-width%3D%221.5%22%20d%3D%22m6%208%204%204%204-4%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[right_0.5rem_center] bg-no-repeat";
const labelCls =
      ...
      {isOpen && (
        ...
        >
          {suggestions.length > 0 ? (
            ...
            <li className="px-4 py-3 text-slate-500 dark:text-slate-500 italic text-sm text-center">
              No matching results
            </li>
          )}
        </ul>
...
const ClassModal = ({ classData, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (classData.schedule) {
      ...
      const type = lower.includes("weekday")
        ? "weekday"
      ...
      const timeMatch = timeSlots.find(
        (s) =>
          s.time && lower.includes(s.time.toLowerCase().replace(/\s/g, "")),
      )?.time;
      setSelectedTime(
        timeMatch ?? classData.schedule.split(" ").slice(1).join(" "),
      );
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.teacherId || !formData.schedule) {
      return setError(
        "Please complete all required fields (Name, Teacher, and Schedule).",
      );
    }
    ...
    >
      <div className="flex items-center gap-3 mb-6 -mt-2">
        ...
        <div>
          <p className="text-xs text-slate-500 font-medium">
            Configure room, teacher, and student enrollment
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label htmlFor="room-name" className={labelCls}>
              Class Name / Room
            </label>
            <select
              id="room-name"
        ...
        <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="relative">
              <label htmlFor="time-slot-select" className="sr-only">
                Time Slot
              </label>
              <select
                id="time-slot-select"
            ...
            onSelect={(s) => {
              if (s.status === StudentStatus.Pending) {
                const proceed = window.confirm(
                  `This student is currently PENDING. Are you sure you want to enroll them?`,
                );
                if (!proceed) return;
              }
              if (selectedStudents.some((x) => x.id === s.id)) {
                setError(
                  `The student "${s.name}" is already in the enrollment list.`,
                );
                return;
              }
              const existingEnr = enrollments.find(
                (e) =>
                  e.studentId === s.id &&
                  (classData ? e.classId !== classData.id : true),
              );
              if (existingEnr) {
                const enrolledClass = classes.find(
                  (c) => c.id === existingEnr.classId,
                );
                setError(
                  `The student name ( ${s.name} ) is already in class ( ${enrolledClass?.name || "another class"} )`,
                );
                return;
              }
                    ...
                    <tr>
                      {["ID", "Name", "Sex", "Phone", ""].map((h) => (
                        <th
                          key={h}
                          scope="col"
                          className="px-4 py-2.5 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {selectedStudents.length > 0 ? (
                      ...
                      <tr>
                        <td
                          colSpan={5}
                          className="text-center py-8 text-sm text-slate-500 italic"
                        >
        ...
        {error && (
          <p
            className="text-sm font-medium text-rose-500 animate-in fade-in slide-in-from-left-2"
            role="alert"
          >
            {error}
          </p>
