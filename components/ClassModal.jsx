    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (classData) {
                ...
                await addClass({ ...formData, id: newClassId });
                await updateClassEnrollments(newClassId, studentIds);
            }
            onClose();
        } catch (err) {
            console.error('Submit error:', err);
            // Context already handles background error, button will revert
        } finally {
            setIsSaving(false);
        }
    };
            ...
            <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-3xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end pt-4 space-x-4">
                        ...
                        <button type="submit" disabled={isSaving} className="px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700 disabled:bg-primary-400 disabled:cursor-not-allowed flex items-center gap-2">
                            {isSaving && <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>}
                            {isSaving ? 'Saving...' : (classData ? 'Update Class' : 'Create Class')}
                        </button>
                    </div>
                </form>
            </div>
