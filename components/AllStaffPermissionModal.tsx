...
const leaveColor = (type: LeaveType) => {
    switch (type) {
...
const AllStaffPermissionModal: React.FC<AllStaffPermissionModalProps> = ({ onClose }) => {
    ...
    const groupedByStaff = useMemo(() => {
        const map = new Map<string, { staffName: string; staffId: string; permissions: typeof staffPermissions }>();
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col max-h-[85vh]">
                ...
                {/* Header */}
                <div className="px-7 py-5 bg-gradient-to-r from-slate-50 to-white flex justify-between items-center border-b border-slate-100 shrink-0">
                    <div>
                        <h2 className="text-xl font-bold text-slate-800 tracking-tight">Permission History</h2>
                        <p className="text-xs text-slate-500 mt-0.5">{groupedByStaff.length} staff with records</p>
                    </div>
                    <button onClick={onClose} className="p-2 bg-white border border-slate-200 hover:bg-slate-100 rounded-full transition-colors text-slate-500 hover:text-slate-800 shadow-sm">
                ...
                {/* List */}
                <div className="overflow-y-auto flex-grow p-4 space-y-2 bg-slate-50/50">
                    {groupedByStaff.length === 0 ? (
                        <div className="text-center py-16 flex flex-col items-center space-y-3 text-slate-400">
                            ...
                            <p className="text-sm font-medium">No permission records yet</p>
                        </div>
                    ) : (
                                ...
                                <div key={staffId} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                                    ...
                                    >
                                        <div className="flex items-center space-x-3 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <span className="font-semibold text-slate-800 text-sm">{staffName}</span>
                                                <div className="flex flex-wrap gap-1 mt-1">
                                                    {uniqueTypes.map(type => (
                                                        <span key={type} className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${leaveColor(type)}`}>
                                                            {type}
                                                        </span>
                                                    ))}
                                                </div>
                                        ...
                                        <div className="flex items-center space-x-2 shrink-0 ml-2">
                                            <span className="text-xs text-slate-400 font-medium">{permissions.length} record{permissions.length !== 1 ? 's' : ''}</span>
                                            <svg
                                    ...
                                    {isExpanded && (
                                        <div className="border-t border-slate-100 divide-y divide-slate-100 animate-in slide-in-from-top-1 duration-200">
                                            {permissions.map(p => (
                                                <div key={p.id} className="px-4 py-3 flex items-start justify-between bg-slate-50/70">
                                                    <div className="space-y-1 flex-1 min-w-0 pr-3">
                                                        <div className="flex items-center space-x-2 flex-wrap gap-1">
                                                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${leaveColor(p.type)}`}>
                                                                {p.type}
                                                            </span>
                                                            <span className="text-xs text-slate-500 font-medium flex items-center">
                                                                <svg className="w-3 h-3 mr-1 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                                    ...
                                                    {deletingPermissionId === p.id ? (
                                                        <div className="flex items-center space-x-1 shrink-0">
                                                            ...
                                                            <button onClick={() => setDeletingPermissionId(null)} className="px-2 py-1 text-[10px] font-bold text-slate-600 bg-slate-200 hover:bg-slate-300 rounded-md transition-colors">No</button>
                                                            <button onClick={() => { deleteStaffPermission(p.id); setDeletingPermissionId(null); }} className="px-2 py-1 text-[10px] font-bold text-white bg-red-600 hover:bg-red-700 rounded-md transition-colors">Yes</button>
                                                        </div>
                                                    ) : (
                                                        <button
                                                            onClick={() => setDeletingPermissionId(p.id)}
                                                            className="shrink-0 p-1 text-red-300 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                                                            title="Delete"
                                                        >
                                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
