...
import { Student, Staff, Class, Page, StaffRole } from '../types';
...
type SearchResult = 
    ...
    | { type: 'teacher'; data: Staff }
    | { type: 'class'; data: Class };

const StudentSearch: React.FC<SearchProps> = ({ navigate }) => {
    const { 
        students, 
        staff,
        classes,
    ...
    useEffect(() => {
        ...
        const teacherResults: SearchResult[] = staff
                ...
                {teacherResults.length > 0 && (
                    <>
                         ...
                         {teacherResults.map((result) => {
                             currentIndex++;
                             const localIndex = currentIndex;
                             const teacher = result.data as Staff;
                             return (
                                <li
                ...
                {classResults.length > 0 && (
                    <>
                         ...
                         {classResults.map((result) => {
                             currentIndex++;
                             const localIndex = currentIndex;
                             const cls = result.data as Class;
                             return (
                                <li
                                    key={`class-${cls.id}`}
                                    onClick={() => handleSelect(result)}
