...
const generateExcelFile = (rows, filename) => {
  ...
  worksheet["!cols"] = [12, 22, 15, 12, 18, 14, 12].map((width) => ({
    wch: width,
  }));
...
const PanelCard = ({ children, className = "" }) => (
  <div
    className={`bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-200/50 dark:border-slate-800 p-8 transition-all duration-500 hover:shadow-2xl ${className}`}
  >
    {children}
  </div>
);
...
const InfoStat = ({ label, value, colorClass }) => (
  <div className="bg-slate-50 dark:bg-slate-950/40 rounded-2xl p-4 border border-slate-100 dark:border-slate-800/50 group hover:bg-white dark:hover:bg-slate-800 transition-colors">
    <p
      className={`text-xl font-black mb-0.5 ${colorClass} group-hover:scale-110 transition-transform`}
    >
      {value}
    </p>
...
const ExportCenter = () => {
  ...
  const targetStudents = useMemo(() => {
    ...
    if (exportMode === "level") {
      ...
      const classIds = new Set(
        classes.filter((c) => c.level === selectedLevel).map((c) => c.id),
      );
      const studentIds = new Set(
        enrollments
  ...
  const handleDownload = () => {
    ...
    const rows = targetStudents.map((s) => {
      ...
      const avg = sGrades.length
        ? sGrades.reduce((acc, g) => acc + g.score, 0) / sGrades.length
        : null;
  ...
  const hasSelection =
    (exportMode === "class" && !!selectedClassId) ||
      ...
      <PanelCard className="flex flex-col justify-center">
        <div className="mb-8">
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
            Configure your parameters to generate official student rosters and
            performance data.
          </p>
        </div>
        ...
        <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div
              className={`w-2 h-2 rounded-full ${hasSelection ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
            />
            <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest">
              {hasSelection ? "Ready for generation" : "Awaiting parameters"}
            </span>
          </div>
          <p className="text-xs font-bold text-slate-600 dark:text-slate-400">
            {targetStudents.length} Record
            {targetStudents.length !== 1 ? "s" : ""} Identified
          </p>
        </div>
      </PanelCard>

      {/* ── PREVIEW & DOWNLOAD PANEL ── */}
      <PanelCard className="flex flex-col items-center justify-center text-center bg-linear-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-950">
        <div
          className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 transition-all duration-500 ${hasSelection ? "bg-primary-600 text-white shadow-2xl shadow-primary-500/40 rotate-0" : "bg-slate-100 dark:bg-slate-800 text-slate-300 -rotate-12"}`}
        >
          {hasSelection ? (
            <svg
              className="w-10 h-10"
        ...
        <div className="px-6 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm mb-8 max-w-full">
          <p className="text-xs font-mono font-bold text-slate-500 dark:text-slate-400 truncate">
            {filename ?? "parameters_not_set.xlsx"}
          </p>
        </div>
