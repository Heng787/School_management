...
const MarksEntry = () => {
  ...
  const handleImportComplete = (importedGrades) => {
    // Merge imported grades into localGrades
    setLocalGrades(prev => {
      const next = { ...prev };
      Object.keys(importedGrades).forEach(studentId => {
        next[studentId] = { ...next[studentId], ...importedGrades[studentId] };
      });
      return next;
    });

    // Mark these students as modified so they can be saved
    setModifiedIds(prev => {
      const next = new Set(prev);
      Object.keys(importedGrades).forEach(studentId => next.add(studentId));
      return next;
    });
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col xl:flex-row items-center justify-between gap-8">
        ...
        {selectedClassId && classStudents.length > 0 && (
          <div className="flex items-center gap-4 w-full xl:w-auto justify-end">
            ...
            <button aria-label="Import marks from Excel" onClick={() => setIsImportModalOpen(true)} className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-primary-600 hover:bg-primary-50 transition-all active:scale-95" title="Import from Kid Scoresheet">
