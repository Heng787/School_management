...
export const configService = {
    getSubjects: async () => {
        ...
        const local = localStore.get<string[]>('subjects', []);
    ...
    saveSubjects: async (subs: string[]) => pushConfig('subjects', subs),

    getLevels: async () => {
        const client = getSupabase();
        const local = localStore.get<string[]>('levels', ['K1', 'K2', 'K3', 'K4', 'K5', 'K6', 'K7', 'K8', 'K9', 'K10']);
        if (!client || !navigator.onLine) return local;
        try {
    ...
    saveLevels: async (lvls: string[]) => pushConfig('levels', lvls),

    getTimeSlots: async () => {
        const client = getSupabase();
        const local = localStore.get<TimeSlot[]>('time_slots', [
    ...
    saveTimeSlots: async (slots: TimeSlot[]) => pushConfig('time_slots', slots),

    getAdminPassword: async () => {
        const client = getSupabase();
        const local = localStore.get<string>('admin_password', 'admin123');
        if (!client || !navigator.onLine) return local;
        try {
    ...
    saveAdminPassword: async (pwd: string) => pushConfig('admin_password', pwd),
};
