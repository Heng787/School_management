...
function App() {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case Page.Settings:
        return (
          <SettingsPage onLogout={handleLogout} userRole={currentUser.role} />
        );
      case Page.Messages:
        return <MessagesPage />;
      default:
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 overflow-hidden transition-colors duration-300">
      ...
      <div
        className={`flex-1 min-w-0 flex flex-col transition-all duration-300 ease-in-out ${isSidebarOpen ? "lg:pl-64" : "pl-0"}`}
      >
        <Navbar
          userRole={currentUser.role}
        ...
        <main
          id="main-content"
          className="flex-1 overflow-x-hidden overflow-y-auto bg-slate-100 dark:bg-slate-900 p-3 sm:p-6 lg:p-10 relative transition-colors duration-300"
        >
